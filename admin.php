        ...
        <div class="donhang">
            <table class="table-header">
                <tr>
                    ...
                    <th title="Sắp xếp" style="width: 13%" onclick="sortDonHangTable('khach')">Khách <i class="fa fa-sort"></i></th>
            ...
            <?php
                ...
                $db = new DB_driver();     
                $i = 1;
                $sql = "SELECT hd.MaHD, ng.Ten, sp.TenSP, hd.TongTien, hd.NgayLap, hd.TrangThai
                FROM hoadon AS hd , chitiethoadon as cthd, sanpham AS sp, nguoidung AS ng
                WHERE hd.MaHD = cthd.MaHD AND sp.MaSP = cthd.MaSP and ng.MaND = hd.MaND";
                $list = $db -> get_list($sql);
                echo "<table class='table-outline hideImg'>";
                foreach ($list as $rowname => $row) {
                    $trangthai = "";
                    if($row['TrangThai'] = '1'){
                        $trangthai = "Duyệt";
                    } else{
                    ...
                    echo "<tr>
                        ...
                        <td style='width: 7%'>" . $row['MaHD'] . "</td>
                        <td style='width: 13%'>
                            ". $row['Ten']."
                        </td>
                        <td style='width: 20%'>" . $row['TenSP'] . "</td>
                        <td style='width: 15%'>" . number_format($row['TongTien'])  . "</td>
                        <td style='width: 10%'>" . $row['NgayLap'] . "</td>
